...
const boolean do_Serial_communication = true; // good for debugging, but makes code slower
...
void loop() {
  ...
  if (enableDataLogging) {
    ...
    if (!SD.exists(filename_log)) {
      ...
      myFile.println(F("time [ms], Set1 [°C], T1[°C], Set2[°C], T2[°C], T3[°C], T4[°C], Cooling power 1 [%], Cooling Power 2 [%], mot_position [mum]"));
    ...
    myFile.print(F(", "));
    myFile.print(peltier1_percentage);
    myFile.print(F(", "));
    myFile.print(peltier2_percentage);
    myFile.print(F(", "));
